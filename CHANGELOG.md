...
- `CumulativeChunker` encodes each split once, in `batch_size` batches, before it walks them. It used to spend two encoder requests per step: one for the next split, and one for the running chunk, which at the start of every chunk is a single split it had just paid for. Both now come out of the batched pass, so the cost is one pass over the splits plus one request per step where the running chunk holds more than one split — where it was two requests per split. On the two-hundred-sentence document the functional suite chunks, that is 102 requests where there were 406. The chunks are unchanged: same cuts, same `triggered_score`, same offsets. The async path issues the batches concurrently rather than one after another.
